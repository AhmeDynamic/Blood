		...
		<div class="row">
			...
			<div class="col-md-12">
				<div class="card">
					...
					<i class="fa fa-plus"></i> اضافة متبرع جديد
				</a></span>
					</div>
					<div class="card-body">
						<table class="table table-condensed table-bordered table-hover">
							<thead>
								<tr>
									...
									<th class="">اسم المتبرع</th>
									<th class="">نوع الدم</th>
									<th class="">معلومات اضافية</th>
									<th class="">حالات التبرع</th>
							...
							<tbody>
								...
								<tr>
									...
									<td class="">
										 <p>البريد الالكتروني: <b><?php echo $row['email']; ?></b></p>
										 <p>رقم الهاتف: <b><?php echo $row['contact']; ?></b></p>
										 <p>العنوان: <b><?php echo $row['address']; ?></b></p>
									</td>
									<td>
										<?php echo !empty($prev) ? date('M d, Y',strtotime($prev)) : 'New' ?>
									</td>
									<td class="text-center">
										<button class="btn btn-sm btn-outline-primary edit_donor" type="button" data-id="<?php echo $row['id'] ?>" >تعديل</button>
										<button class="btn btn-sm btn-outline-danger delete_donor" type="button" data-id="<?php echo $row['id'] ?>">مسح</button>
									</td>
								</tr>
								<?php endwhile; ?>
							</tbody>
...
<script>
	...
	$('.delete_donor').click(function(){
		_conf("هل انت متاكد انك تريد حدف هذا?","delete_donor",[$(this).attr('data-id')])
	})
	...
	function delete_donor($id){
		...
		$.ajax({
			...
			success:function(resp){
				if(resp==1){
					alert_toast("تم المسح بنجاح",'success')
					setTimeout(function(){
						location.reload()
					},1500)
