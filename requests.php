						...
						<table class="table table-condensed table-bordered table-hover">
							<thead>
								<tr>
									...
									<th class="">كود الطلب </th>
									<th class="">اسم المريض</th>
									<th class="">نوع الدم</th>
									<th class="">معلومات اضافة</th>
									<th class="">حالة الطلب</th>
							...
							<tbody>
								...
								<tr>
									...
									<td class="">
										 <p>الكمية المطلوبة: <b><?php echo  ($row['volume'] / 1000).' L' ?></b></p>
										 <p>اسم الطبيب: <b><?php echo ucwords($row['physician_name']) ?></b></p>
									</td>
									...
									<td class="text-center">
										<button class="btn btn-sm btn-outline-primary edit_request" type="button" data-id="<?php echo $row['id'] ?>" >تعديل </button>
										<button class="btn btn-sm btn-outline-danger delete_request" type="button" data-id="<?php echo $row['id'] ?>">حذف</button>
									</td>
								</tr>
								<?php endwhile; ?>
							</tbody>
...
<script>
	...
	$('.delete_request').click(function(){
		_conf("هل انت متاكد من حذف هذا ?","delete_request",[$(this).attr('data-id')])
	})
	...
	function delete_request($id){
		...
		$.ajax({
			...
			success:function(resp){
				if(resp==1){
					alert_toast("تم الحذف بنجاح",'success')
					setTimeout(function(){
						location.reload()
					},1500)
